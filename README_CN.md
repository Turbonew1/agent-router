...
**Agent Router** 扫描你已安装的 skills、agents 和 MCP 工具，然后生成一个**智能路由表**，告诉 AI Agent 每种任务该用哪个工具。
...
Agent Router 创建一个映射：**任务模式 → 最佳工具**。它自动加载到每个会话中，你不需要输入任何命令 — 直接描述任务即可。
...
### Q: 路由选错了工具怎么办？
**A:** 编辑 `~/.claude/rules/common/agent-router.md` 调整优先级，或者说"用 X skill"来覆盖。
