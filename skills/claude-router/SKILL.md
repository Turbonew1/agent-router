...
# Claude Router — 智能任务路由 / Intelligent Task Router

> One skill to rule them all — automatically routes your tasks to the right skill, agent, or MCP tool.

## What This Does

Claude Router scans your installed tools and creates a **smart routing table** that tells Claude which tool to use for each task type. No more guessing — just describe your task and the right tool fires automatically.

## When to Run

- **First time**: Run `/claude-router` after installing this skill
...
- **Wrong tool selected**: Edit `~/.claude/rules/common/claude-router.md` to adjust priorities
- **Tool not found**: Re-run scan to pick up newly installed tools
- **Rules not loading**: Ensure the file is at `~/.claude/rules/common/claude-router.md`
